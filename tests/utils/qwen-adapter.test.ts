...
import { QwenAdapter } from '../../src/utils/adapters/qwen-adapter'
...
describe('QwenAdapter', () => {
  ...
  describe('streamChat error handling', () => {
    it('should throw on API error response', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Qwen API error: 401')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
    ...
    it('should throw on 500 server error', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Qwen API error: 500')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe('streamChat URL construction', () => {
    it('should append /chat/completions if not present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1' })
        const adapter = new QwenAdapter(config)
    ...
    it('should not duplicate /chat/completions if already present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions' })
        const adapter = new QwenAdapter(config)
        await collectChunks(adapter.streamChat([]))
