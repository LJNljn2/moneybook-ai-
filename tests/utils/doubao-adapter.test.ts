...
import { DoubaoAdapter } from '../../src/utils/adapters/doubao-adapter'
...
function createTestConfig(overrides: Partial<ProviderConfig> = {}): ProviderConfig {
  const platform: AiPlatform = {
    ...
    models: [
      { name: 'doubao-lite', supportsThinking: false },
      { name: 'doubao-pro-32k', supportsThinking: true },
    ],
    isCustom: false,
  }
...
describe('DoubaoAdapter', () => {
  ...
  describe('streamChat with thinking field', () => {
    ...
    it('should handle both thinking and content in same delta', async () => {
      // Some responses may send both fields in the same chunk
      const sseChunks = [
  ...
  describe('streamChat error handling', () => {
    it('should throw on API error response', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Doubao API error: 401')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
    ...
    it('should throw on 500 server error', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Doubao API error: 500')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe('streamChat URL construction', () => {
    it('should append /chat/completions if not present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://ark.cn-beijing.volces.com/api/v3' })
        const adapter = new DoubaoAdapter(config)
    ...
    it('should not duplicate /chat/completions if already present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://ark.cn-beijing.volces.com/api/v3/chat/completions' })
        const adapter = new DoubaoAdapter(config)
        await collectChunks(adapter.streamChat([]))
