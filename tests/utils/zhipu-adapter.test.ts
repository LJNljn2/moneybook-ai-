...
import { ZhipuAdapter } from '../../src/utils/adapters/zhipu-adapter'
...
describe('ZhipuAdapter', () => {
  ...
  describe('streamChat with reasoning_content', () => {
    ...
    it('should handle both reasoning_content and content in same delta', async () => {
      // Some models may send both fields in the same chunk
      const sseChunks = [
  ...
  describe('streamChat error handling', () => {
    it('should throw on API error response', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Zhipu API error: 401')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
    ...
    it('should throw on 500 server error', async () => {
      ...
      try {
        ...
        await expect(collectChunks(iterator)).rejects.toThrow('Zhipu API error: 500')
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe('streamChat URL construction', () => {
    it('should append /chat/completions if not present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://open.bigmodel.cn/api/paas/v4' })
        const adapter = new ZhipuAdapter(config)
    ...
    it('should not duplicate /chat/completions if already present', async () => {
      ...
      try {
        const config = createTestConfig({ baseUrl: 'https://open.bigmodel.cn/api/paas/v4/chat/completions' })
        const adapter = new ZhipuAdapter(config)
        await collectChunks(adapter.streamChat([]))
  ...
  describe('streamChat request format', () => {
    ...
    it('should send messages in OpenAI format', async () => {
      ...
      try {
        ...
        expect(body.model).toBe('glm-4-reasoning')
